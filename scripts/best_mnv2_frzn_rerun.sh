...
#SBATCH --job-name=coast_mnv2_frzn
#SBATCH --output=./out/jobs/coast_mnv2_frzn-%j.out
...
module_unfreeze_depth=0
CHECKPOINT_DIR=none

# Create output directory
TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)
OUTPUT_DIR=out/bests/mnv2_frzn_$TIMESTAMP
mkdir -p ${OUTPUT_DIR}

# Run the training script
srun --time=2:00:00 python src/train.py \
    --config-dir configs \
    --config-name COASTMobileNetV2_frozen.yaml \
