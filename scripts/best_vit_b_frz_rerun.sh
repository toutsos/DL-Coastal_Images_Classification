...
#SBATCH --job-name=coast_vit_frozen
#SBATCH --output=./out/jobs/best_coast_vit_frozen-%j.out
...
OUTPUT_DIR=out/bests/vit_b_frzn_$TIMESTAMP
mkdir -p ${OUTPUT_DIR}

# Run the training script
srun --time=2:00:00 python src/train.py \
    --config-dir configs \
    --config-name COAST_vit_b_16_frozen.yaml \
