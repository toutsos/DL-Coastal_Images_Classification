...
#SBATCH --job-name=coast_mnv2_finetune
#SBATCH --output=./out/jobs/coast_mnv2_finetune-%j.out
...
datamodule_batch_size=64
datamodule_rotation=20
module_lr=0.01
module_momentum=0.7
module_weight_decay=0.001
module_dropout=0.6
module_n_layer1=768
module_n_layer2=128
module_batch_norm=true
module_unfreeze_depth=12
CHECKPOINT_DIR=/home/roberto.milanese/DeepLearning/DL-Coastal_Images_Classification_2/out/bests/mnv2_frzn_2025-02-06_21-08-11/lightning_logs/version_0/checkpoints/last.ckpt
# Create output directory
TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)
OUTPUT_DIR=out/bests/mnv2_ft_$TIMESTAMP
mkdir -p ${OUTPUT_DIR}

# Run the training script
srun --time=2:00:00 python src/train.py \
    --config-dir configs \
    --config-name COASTMobileNetV2_unfrozen.yaml \
