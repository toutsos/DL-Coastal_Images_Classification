...
class COASTMobileNetV2(pl.LightningModule):
    def __init__(self, **config):
        ...
        input_features = self.backbone.last_channel  # MobileNetV2's actual feature size
    ...
    def forward(self, x, return_features=False, return_fixed_features=False):
        ...
        print(f"Shape of x after backbone: {x.shape}")
        x2 = self.classifier(x)
        # x = self.pooling(x).squeeze()  # Apply pooling , already included in the ResNet-50 model
        # x = self.classifier(x)

        if return_features:
            return x2[:, :-1]  # Extract second-to-last layer
        if return_fixed_features:
            return x
        
        return x2 # Pass input features through model
